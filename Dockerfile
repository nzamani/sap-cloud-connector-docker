...
# General information
################################################################
LABEL com.nabisoft.sapcc.version="2.11.0.3"
...
# following command (then retry the next steps):
...
RUN wget --no-check-certificate --no-cookies --header "Cookie: eula_3_1_agreed=tools.hana.ondemand.com/developer-license-3_1.txt; path=/;" -S https://tools.hana.ondemand.com/additional/sapcc-2.11.0.3-linux-x64.zip && \
    wget --no-check-certificate --no-cookies --header "Cookie: eula_3_1_agreed=tools.hana.ondemand.com/developer-license-3_1.txt; path=/;" -S https://tools.hana.ondemand.com/additional/sapjvm-8.1.035-linux-x64.rpm && \
    unzip sapcc-2.11.0.3-linux-x64.zip && \
    rpm -i sapjvm-8.1.035-linux-x64.rpm && rpm -i com.sap.scc-ui-2.11.0-7.x86_64.rpm
...
#HINT: the command above is basically derived from the SAPCC "portable" archives which can be
# downloaded from https://tools.hana.ondemand.com/#cloud, i.e. sapcc-2.11.0.3-windows-x64.zip, sapcc-2.11.0.3-linux-x64.tar.gz, sapcc-2.11.0.3-macosx-x64.tar.gz
# To verify this simply extract any of these archives and check the file "props.ini"
