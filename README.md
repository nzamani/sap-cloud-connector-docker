...
1. Build the Docker image
    ...
    - Without Proxy
        ...
        ```sh
        docker build -t sapcc:2.12.1.1 .
        ```

    - Behind a Proxy

        ```sh
        docker build --build-arg http_proxy=http://proxy.mycompany.corp:1234 --build-arg https_proxy=http://proxy.mycompany.corp:1234 -t sapcc:2.12.1.1 .
        ```
...
1. Create a container running as a deamon
    ...
    - Use this if you want to map the default SAP ports as they come on localhost (preferred)
        ...
        ```sh
        docker run -p 8443:8443 -h mysapcc --name sapcc -d sapcc:2.12.1.1
        ```

    - Use this one if "random" ports on localhost are fine for you

        ```sh
        docker run -P -h mysapcc --name sapcc -d sapcc:2.12.1.1
        ```
